...
@section('title')
    <title>Supplier - Sistem Informasi Koperasi dan Usaha</title>
@endsection
@section('page_css')
      ...
      <form action="{{ route('supplier.import') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body"> 
          <div class="form-group mb-3">
            <label for="supplierFile"> Format file harus sesuai template</label>
            <div class="custom-file">
              <input type="file" class="custom-file-input" id="supplierFile" name="file" accept=".xlsx,.xls" required>
              <label class="custom-file-label" for="supplierFile">Choose file</label>
            </div>
          </div>
        </div>
...
<div class="container-fluid">
    <div class="row justify-content-center">
      <div class="col-12">
        <div class="col">
            <h2 class="h3 mb-0 page-title">Daftar Supplier</h2>
        </div>
        <div class="row align-items-center my-4">
            <div class="col">
                <a href="{{ route('supplier.create') }}" class="btn mb-2 btn-primary">
            ...
            <div class="col-auto">
              <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton" style="">
                  <a class="dropdown-item" href="{{ route('supplier.template')}}"><small>Download Template</small></a>
                  ...
                  <tbody>
                    ...
                    @foreach ($supplier as $spl)
                      <tr>
                        ...
                        <td>{{ $spl->name }}</td>
                        <td>{{ $spl->address }}</td>
                        <td>{!! $spl->is_active == 1 ? "<span class='dot dot-lg bg-success mr-1'></span>Aktif" : "<span class='dot dot-lg bg-secondary mr-1'></span>Tidak Aktif" !!}</td>
                        <td><button class="btn btn-sm dropdown-toggle more-horizontal" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <span class="text-muted sr-only">Action</span>
                          </button>
                          <div class="dropdown-menu dropdown-menu-right">
                            <a class="dropdown-item" href="{{ route('supplier.edit', $spl->id) }}">Edit</a>
                            <form action="{{ route('supplier.destroy', $spl->id) }}" method="POST" style="display: inline;" id="deleteForm">
                                @csrf
                                @method('DELETE')
