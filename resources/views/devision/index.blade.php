...
@section('title')
    <title>Bagian - Sistem Informasi Koperasi dan Usaha</title>
@endsection
@section('page_css')
      ...
      <form action="{{ route('devisions.import') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body"> 
          <div class="form-group mb-3">
            <label for="devisionFile"> Format file harus sesuai template</label>
            <div class="custom-file">
              <input type="file" class="custom-file-input" id="devisionFile" name="file" accept=".xlsx,.xls" required>
              <label class="custom-file-label" for="devisionFile">Choose file</label>
            </div>
          </div>
        </div>
...
<div class="container-fluid">
    <div class="row justify-content-center">
      <div class="col-12">
        <div class="col">
            <h2 class="h3 mb-0 page-title">Daftar Bagian</h2>
        </div>
        ...
        <div class="row my-4">
          <div class="col-md-12">
            <div class="card shadow">
              <div class="card-body">
                <table class="table datatables" id="devision">
                  <thead>
                    <tr>
                      <th width="5%">No.</th>
                      <th width="15%">Kode</th>
                  ...
                  <tbody>
                    ...
                    @foreach ($devision as $dvs)
                      <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $dvs->code }}</td>
                        <td>{{ $dvs->name }}</td>
                        <td>{{ $dvs->remark }}</td>
                        <td>{!! $dvs->is_transactional == 1 ? "<span class='dot dot-lg bg-success mr-1'></span>Aktif" : "<span class='dot dot-lg bg-secondary mr-1'></span>Tidak Aktif" !!}</td>
...
<script>
    ...
    $('#btnDelete').on('click', function(e) {
      if (!confirm('Anda yakin ingin mengubah Aktivasi Bagian ini?')) {
        e.preventDefault();
      } else {
        $('#deleteForm').submit()
      }
    ...
    $('#devisionFile').on('change', function() {
